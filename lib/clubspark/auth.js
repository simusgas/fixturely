...
/**
 * Get the browser launch options depending on the environment.
 * - Local dev: use system Chrome via channel: 'chrome'
 * - Vercel: use @sparticuz/chromium's bundled binary
 */
async function getBrowserOptions() {
  if (IS_VERCEL) {
    const sparticuzChromium = await import('@sparticuz/chromium')
    const chromiumMod = sparticuzChromium.default
    const execPath = await chromiumMod.executablePath()
    console.log('[ClubSpark Auth] Chromium path:', execPath)
    return {
